...
public class MapToContactEntityTests
{
    ...
    [Fact]
    public void ToContactEntity_WithNullSource_ShouldReturnNull()
    {
        var result = MapToContactEntity.ToContactEntity(null!);
    ...
    [Fact]
    public void ToContactEntity_WithNullSource_Should()
    {
        ...
        contact.ToContactEntity(null!);

        Assert.Equal(100, contact.ContactId);
        Assert.Equal("Modified Test", contact.FirstName);
        Assert.Equal("Modified User", contact.LastName);
        Assert.Equal("test.user@example.com", contact.Email);
        Assert.Equal("active", contact.Status);
        Assert.Equal("Test Type", contact.Type);
    }
}
